...
/* Optimized SHA256 for 80-byte Bitcoin headers using midstate */
void sha256d_80_neon(const uint8_t *header, uint8_t *digest)
{
    ...
    /* Output first hash in big-endian */
    for (i = 0; i < 8; i++) {
        be32enc(hash1 + i * 4, state[i]);
    }
    ...
    /* Length = 32 * 8 = 256 bits = 0x100 */
    block[62] = 0x01;
    block[63] = 0x00;
    sha256_transform(state, block);

    /* Output final hash in big-endian */
    for (i = 0; i < 8; i++) {
        be32enc(digest + i * 4, state[i]);
    }
}

/* SHA256d using precomputed midstate - for mining inner loop */
void sha256d_ms_neon(const uint32_t *midstate, const uint8_t *tail, uint8_t *digest)
{
    ...
    /* First SHA256 pass - continue from midstate */
    memcpy(state, midstate, 32);

    /* Build second block: last 16 bytes + padding */
    memset(block, 0, 64);
